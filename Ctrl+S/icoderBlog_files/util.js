...
var cookie_vip_key="irV8jfnifwlj9o0Y";var cookie_vip_val="iGL4OvQbzTfToq5m";var key='doubi';var map={'juejin':'掘金','oschina':'开源中国','sjdbc':'Sharding-JDBC','jianshu':'简书','csdn':'CSDN','iteye':'iteye','cnblogs':'博客园','bilibili':'B 站'};var unlockUrls=['zhishixingqiu','Spring/good-collection','Spring-Boot/good-collection','Spring-MVC/good-collection','MyBatis/good-collection','Dubbo/good-collection','Netty/Netty-collection','Interview/good-collection','Entity/good-collection','chinese-comment-repository','Architecture/how-to-read-source-code',];var closeArticleHeaderFlag=true;var closeVipAlert=true;function isVIP(){var vip=false;if(location.hostname.indexOf('vip')>=0){vip=true;}
...
function removeArticleHeader(){var tip=$('blockquote');if(tip.html&&tip.html()&&tip.html().indexOf&&tip.html().indexOf('关注')>0){tip.prev('p').remove();tip[0].remove();}}
...
return false;};}}});$('#read_more').click(function(){var lockWin=jqueryAlert({'title':'首次访问，人机识别','width':'500','height':'590','modal':true,'bodyScroll':'true','content':'扫描下方二维码回复 <strong>可口可乐</strong> 获取解锁验证码'
+'\n<img width="400" src="http://www.iocoder.cn/images/common/wechat_mp_simple.png" />'
...
+'\n验证码：<input type="text" id="unlockCode" placeholder="请输入解锁验证码">（请输入验证码）','buttons':{'提交':function(){var code=$('#unlockCode').val();if(code.indexOf('coke')>=0){alert('恭喜，解锁成功！\n希望胖友在技术的道路上：\n* 知其然，知其所以然。\n* 人若无名，专心练剑。');$.cookie(cookie_vip_key,cookie_vip_val,{expires:365,path:'/'});removeArticleHeader();removeLocker();lockWin.close();location.reload();}else{alert('解锁验证码有误，请重新输入');}}}});});removeCategoriesPageTitle();if(isDomainVIP()){var search=location.search;if(search&&search.length>0){search+='&vip';}else{search+='?vip';}
